defmodule Boomba.Cooldown do
  ...
  def handle_call({:execute, command, user}, _from, state) do
    case {Map.has_key?(state, "#{command._id}-global"),
          Map.has_key?(state, "#{command._id}-#{user}")} do
      {_, true} ->
        {:reply, :user_lock, state}

      {true, _} ->
        {:reply, :global_lock, state}

      _ ->
        state = Map.put(state, "#{command._id}-global", true)

        :timer.send_after(
          :timer.seconds(command.cooldown.global),
          {:clear_lock, "#{command._id}-global"}
        )
